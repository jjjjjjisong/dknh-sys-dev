create table if not exists public.order_book (
  ...
  constraint order_book_status_check check (status in ('ST00', 'ST01')),
  constraint order_book_shipped_status_check check (shipped_status in ('미출고', '출고')),
  constraint order_book_del_yn_check check (del_yn in ('Y', 'N'))
);
